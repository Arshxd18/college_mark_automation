...
console.log("=== First 8 rows and 25 columns of Internal1_template.xlsx ===");
for (let i = 0; i < Math.min(json1.length, 10); i++) {
    const row = json1[i];
    console.log(`Row ${i}:`, row ? row.slice(0, 25) : null);
}
...
console.log("\n=== First 8 rows and 25 columns of Internal2_assesment.xlsx ===");
for (let i = 0; i < Math.min(json2.length, 10); i++) {
    const row = json2[i];
    console.log(`Row ${i}:`, row ? row.slice(0, 25) : null);
}
